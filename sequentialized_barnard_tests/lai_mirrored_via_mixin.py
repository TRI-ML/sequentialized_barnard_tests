...
'''
class MirroredLaiTest(MirroredTestMixin, SequentialTestBase):
    """A pair of one-sided Lai tests with mirrored alternatives.
    ...
    def calibrate_c(self, n_calibration_trajectories: Optional[int] = 10000) -> None:
        """Calibrate the value of c for each constituent test.

        Args:
            n_calibration_trajectories (Optional[int], optional): Number of calibration trajectories. Defaults to 10000.
        """
        self._test_for_alternative.calibrate_c(n_calibration_trajectories)
        self._test_for_null.calibrate_c(n_calibration_trajectories)

    def set_c(self, alternative_c: float, null_c: Optional[float] = None) -> None:
        """Manually set the value of c for each of the mirrored alternatives.

        Args:
            alternative_c (float): Value of c to set for the test for alternative.
            null_c (Optional[float], optional): Value of c to set for the test for null. If None,
                                                _test_for_null uses alternative_c. Defaults to None.
        """
        self._test_for_alternative.set_c(alternative_c)
        if null_c is not None:
            self._test_for_null.set_c(null_c)
        else:
            self._test_for_null.set_c(alternative_c)
'''
